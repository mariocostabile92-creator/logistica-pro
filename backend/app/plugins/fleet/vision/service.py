...
def fleet_vision(vehicle_id: int | None = None, organization_id: str | None = None) -> dict:
    ...
    for asset in data["assets"]:
        ...
        items.append({
            **asset,
            ...
            "document_count": len(documents[asset_id]),
            "missing_documents": _missing_documents(documents[asset_id]),
            "insurance": policy,
            "franchise_count": len(asset_franchises),
    ...
    return {
        ...
        "summary": {
            ...
            "documents_registered": sum(item["document_count"] for item in items),
            "insurance_policies": sum(bool(item["insurance"]) for item in items),
            "open_franchises": sum(item["franchises_open"] for item in items),
