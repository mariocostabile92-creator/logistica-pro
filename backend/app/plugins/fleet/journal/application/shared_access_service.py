...
def get_active(organization_id: str) -> dict | None:
    item = shared_access_repository.active(organization_id)
    return _present(item) if item else None


def create(organization_id: str, regenerate: bool = False) -> dict:
    current = shared_access_repository.active(organization_id)
    if current and not regenerate:
        return _present(current)
    if current:
        shared_access_repository.revoke(current["id"], _now(), organization_id)
...
def revoke(access_id: str, organization_id: str) -> dict:
    item = shared_access_repository.revoke(access_id, _now(), organization_id)
    if not item:
...
def validate(token: str, *, touch: bool = True) -> dict:
    ...
    if item.get("expires_at") and datetime.fromisoformat(item["expires_at"]) <= datetime.now(timezone.utc):
        raise SharedAccessError("Link condiviso scaduto.", 410)
    if touch:
