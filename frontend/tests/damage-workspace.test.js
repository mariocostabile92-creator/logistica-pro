...
test("damage workspace exposes real KPIs search combinable filters and inline detail", async () => {
  ...
  for (const label of [
    "Pratiche aperte", "Da valutare", "In riparazione",
    "Veicoli fermi", "Chiuse", "Costo stimato totale",
  ]) assert.match(module, new RegExp(label));
  for (const filter of [
    "plate", "status", "severity", "stopped", "origin", "attachments",
  ]) assert.match(module, new RegExp(`data-damage-filter="${filter}"`));
  assert.match(module, /data-damage-reset/);
...
test("damage workflow uses API persistence and accessible controls", async () => {
  const [api, module, css, documents, loader] = await Promise.all([
