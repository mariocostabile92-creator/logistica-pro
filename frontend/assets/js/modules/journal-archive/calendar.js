...
export function archiveCalendar(month, selectedDate, days = []) {
  ...
  for (let day = 1; day <= last; day += 1) {
    ...
    const summary = metrics
      ? `${metrics.total} procedure, ${metrics.anomalies} anomalie, ${metrics.incomplete} incomplete`
      : "Nessuna procedura";
    cells.push(`<button type="button" class="gdb-calendar-day ${date === selectedDate ? "active" : ""} ${metrics ? "has-events" : ""}"
      ...
      <strong>${day}</strong>${metrics ? `<span>${metrics.total}</span><small>${metrics.anomalies ? `${metrics.anomalies} anomalie` : "Regolare"}</small>` : `<small>—</small>`}
    </button>`);
  }
