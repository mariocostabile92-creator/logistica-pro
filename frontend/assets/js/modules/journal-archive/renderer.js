...
export function archiveShell() {
  return `<section class="gdb-calendar-panel" aria-busy="true">
    <header><div><p class="eyebrow">Storico operativo</p><h3>Archivio GDB</h3></div>
    ...
    <div data-gdb-calendar><div class="gdb-loading">Caricamento calendario…</div></div></section>
    <section class="gdb-day-panel" aria-busy="true"><header><div><p class="eyebrow">Giorno selezionato</p><h3 data-gdb-day-title></h3></div></header>
      ...
      <form class="gdb-filters" data-gdb-filters><label>Ricerca<input name="search" type="search" placeholder="Targa, driver, note, ID"></label>
        ...
        <label>Stato<select name="status"><option value="">Tutti</option><option value="completed">Complete</option><option value="in_progress">Incomplete</option></select></label>
...
export function renderDay(root, state) {
  ...
  const kpis = [["total", "Totali", ""], ["check_outs", "Prese in carico", "check_out"], ["check_ins", "Rientri", "check_in"], ["complete", "Complete", "completed"], ["incomplete", "Incomplete", "in_progress"], ["with_anomalies", "Con anomalie", "anomaly"], ["with_media", "Con media", "media"]];
  root.querySelector("[data-gdb-kpis]").innerHTML = kpis.map(([key, label, filter]) => `<button type="button" data-gdb-kpi="${filter}"><strong>${data.summary[key]}</strong><span>${label}</span></button>`).join("");
