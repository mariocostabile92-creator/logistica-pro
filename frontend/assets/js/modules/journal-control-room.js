...
function card(item) {
  return `<button type="button" class="jcr-item ${state.selected?.id === item.id ? "active" : ""}" data-jcr-id="${escapeHtml(item.id)}">
    ...
    <small>${escapeHtml(when(item.occurred_at))} · ${escapeHtml(item.declared_driver_identifier)}</small>
...
function detail(item) {
  ...
  return `<button type="button" class="quiet jcr-back" data-jcr-back>← Torna alla lista</button>
    ...
    <section><h4>Checklist e dotazioni</h4><ul>${equipment}</ul></section>
...
document.addEventListener("click", (event) => {
  ...
  if (vehicle) { root().hidden = true; document.dispatchEvent(new CustomEvent("fleet:vehicle-open", { detail: { assetId: Number(vehicle) } })); }
